...
| **`INSTRUMENT`** | `INST_NUM`, `INST_TYPE`, `NAME`, `TRANSPOSE`, `TABLE`, `PITCH`, `FINETUNE`, `ALGO`, `OP<1..4>_RATIO`, `OP<1..4>_LEVEL`, `OP<1..4>_FEEDBACK`, `OP<1..4>_MOD`, `OP<1..4>_SHAPE`, `OP<1..4>_DETUNE`, `OP<1..4>_WAVE`, `SAMPLE`, `SLICE`, `PLAY_MODE`, `START`, `LOOP_START`, `LENGTH`, `DEGRADE`, `FILTER_TYPE`, `CUTOFF`, `RESONANCE`, `AMP`, `LIMIT`, `VOLUME`, `PAN`, `DRY`, `CHORUS`, `DELAY`, `REVERB` | Complete multi-column coverage for all FM Synth (4-operator matrix, algorithm, feedback, ratios, levels, modulators), Sampler, Wavsynth, Macro, Hyper, and MIDI instrument parameters, header metadata, slicing, and filter routing. |
